...
export default function DualCard({ cameras }: Props) {
  ...
  const left = cameras.find((c) => c.id === 'cam2') // NW-Front → L
  const right = cameras.find((c) => c.id === 'cam1') // SE-Driveway → R
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full" style={{ aspectRatio: '704/240' }}>
        <HlsPlayer
          src={hlsUrl('cam2', 'thumb')}
          startMuted
          objectFit="cover"
          className="absolute inset-0 w-full h-full"
        />
        <div className="absolute bottom-1.5 left-2 z-10 flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/60">
          <span className="text-[10px] font-bold text-indigo-400">L</span>
          <span className="text-[10px] text-zinc-300">{left.name}</span>
        </div>
      </div>
      ...
      <div className="relative w-full" style={{ aspectRatio: '704/240' }}>
        <HlsPlayer
          src={hlsUrl('cam1', 'thumb')}
          startMuted
          objectFit="cover"
          className="absolute inset-0 w-full h-full"
        />
        <div className="absolute bottom-1.5 right-2 z-10 flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/60">
          <span className="text-[10px] text-zinc-300">{right.name}</span>
          <span className="text-[10px] font-bold text-indigo-400">R</span>
        </div>
      </div>
      ...
      <div className="absolute top-2.5 right-2.5 z-10">
        <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider bg-indigo-950/70 text-indigo-400 border border-indigo-800/50">
          Stereo
        </span>
      </div>
      ...
      <div className="px-3 py-2.5 flex items-center justify-between">
        ...
        <span className="text-xs text-zinc-500">
          <span className="text-indigo-500">L</span> · {left.name}&nbsp;·&nbsp;{right.name} · <span className="text-indigo-500">R</span>
        </span>
      </div>
    </div>
  )
