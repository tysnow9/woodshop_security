...
- **R/M overlap resolution** — `GET /api/recordings` trims each segment's `EndTime` to the next segment's `StartTime` when they overlap (Amcrest R clips have a ~5s tail past the M clip start, caused by Anti-Dither). Segments trimmed to < 5 seconds are dropped entirely. Frontend `onTimeUpdate` in CameraPage advances to the next segment early when `video.currentTime >= durationSec - 0.1s`, so the extra tail in the video file is never played. Applies to all transition types (R→M, M→R, R→R, M→M).
- **Playback UI** — fully working Timeline scrubber in CameraPage:
  ...
  - Date picker replaced with `<select>` populated from `availableDates` state (API `listDates` + 60s poll); fixes Safari native calendar popup clipping below viewport; labels like "Wed, Apr 1"
  - Time display (click-to-edit HH:MM:SS, Enter seeks + snaps timeline), speed buttons (0.5×–8×), Go Live — all in one row above the bar; bar is full width
  - Keyboard: Space play/pause, J −10s, K reset speed, L faster; **←/→ tap** = step 1 frame (1/20s); **←/→ hold** = play forward/reverse at current speed (keyup stops); **⇧←/⇧→** = snap to prev/next motion clip midpoint; Home first clip, End go live
  - Reverse playback transitions through clip boundaries: at `currentTime <= stepSec`, loads previous segment at its end via `pendingReverseRef` + `setIsReversing(true)` in `onCanPlay`
  ...
  - Timeline zoom (shift+wheel) anchors to needle — `viewCenterMsRef` unchanged during zoom so the current playback position stays pinned at center
  - Double-click on timeline bar seeks to clicked time and centers needle there

### ⚠️ Known Issues
- **Recorded playback audio/video sync** — audio occasionally runs ~1 second ahead of video in recorded clips. Likely caused by the camera's non-monotonic RTSP timestamps creating drift between the video and audio tracks that `aresample=async=1000` doesn't fully compensate for in the recorded files (it helps the live stream but the NFS recordings are written by the camera directly). Low priority; worth revisiting if it gets worse.
...
- `src/pages/CameraPage.tsx` — full camera view with live/playback modes; loads ALL available dates' segments on mount (`Promise.all` across all dates), polls every 60s for today's new segments; `allSegments` sorted array passed to Timeline; HLS player kept mounted but opacity-0 during playback (no rebuffering on switch back); playback `<video>` also opacity-0 while scrubbing (last decoded frame stays visible under sprite overlay); `canplay` timeout (5s) prevents infinite spinner on corrupt files; `handleSeek` uses 30s threshold — no snap to distant segment; `onViewCenterChange` from Timeline drives time display and time-edit; controls (date `<select>`, playback buttons, time, speed buttons, Go Live) in `headerContent`; playback controls: ⏪ reverse (200ms interval, steps back `playbackRate×0.2s`, transitions to previous segment at boundary via `pendingReverseRef`), ▶/⏸ play-pause, ⏩ forward, ? keyboard shortcut tooltip; `isReversing` cleared on seek/goLive/mode-change; next segment preloaded when <30s remaining; stale-closure–safe refs: `playbackTimeRef` (synced to `playbackTime` + set synchronously in `handleSeek`), `currentSegmentRef`, `allSegmentsRef`, `pendingReverseRef`; `availableDates` state fed from `listDates` + poll; HlsPlayer handle: `setMuted`, `pause`, `resume`
- `src/components/Timeline.tsx` — RAF-based scrubber; hot-path refs + RAF flush pattern (no React re-renders during interaction); `minMs`/`maxMs` props; `jumpToMs` prop snaps view; `headerContent` slot above bar; `userHasPannedRef` blocks live-tick and playback auto-scroll from overriding manual pan; label intervals auto-scale with zoom (60s–10800s); min segment width 2px; needle white/90 playback, white/20 live; live edge red dot; 5 nearest sprites pre-fetched; wheel debounce 500ms; shift+wheel zoom anchors to needle (viewCenter unchanged); double-click seeks to clicked time and centers needle
